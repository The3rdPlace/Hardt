...
int main(int argc, char** argv)
{
    ...
    HWavReader<int16_t> input("wav input", argv[1]);
    ...
    HHumFilter<int16_t> humFilter("humfilter", &input, H_SAMPLE_RATE_48K, 50, 500, BLOCKSIZE);

    // Increase signal strength after mixing to avoid losses before filtering and mixing
    HGain<int16_t> gain("gain", humFilter.Reader(), 100, BLOCKSIZE);
    ...
    HBiQuadFilter<HHighpassBiQuad<int16_t>, int16_t> highpass("highpass filter", gain.Reader(), LOCAL_OSCILATOR, H_SAMPLE_RATE_48K, 0.7071f, 1, BLOCKSIZE);

    // Mix down to the output frequency.
    // 17200Hz - 16160Hz = 1040Hz  (place it somewhere inside the bandpass filter pass region)
    HMultiplier<int16_t> multiplier("multiplier", highpass.Reader(), H_SAMPLE_RATE_48K, LOCAL_OSCILATOR, 10, BLOCKSIZE);
    ...
    HCascadedBiQuadFilter<int16_t> bandpass("bandpass", multiplier.Reader(), coeffs, 20, BLOCKSIZE);

    // General lowpass filtering after mixing down to IF
    HBiQuadFilter<HLowpassBiQuad<int16_t>, int16_t> lowpass("lowpass", bandpass.Reader(), 2000, H_SAMPLE_RATE_48K, 0.7071f, 1, BLOCKSIZE);

    // Final boost of signal (output volume)
    HGain<int16_t> volume("volume", lowpass.Reader(), 20, BLOCKSIZE);
    ...
    HStreamProcessor<int16_t> processor("processor", volume.Reader(), BLOCKSIZE, &terminated);
    ...
    HSplitter<int16_t> splitter("splitter", processor.Consumer());
    ...
    HSignalLevelOutput<int16_t> level("signallevel output", splitter.Consumer(), 2);
    HCustomWriter<HSignalLevelResult>* result = HCustomWriter<HSignalLevelResult>::Create("signallevel result", callback, level.Consumer());
    ...
    HFade<int16_t> fade("fade", splitter.Consumer(), 0, 1000, true, BLOCKSIZE);

    // Create a soundcard writer, to output the final decoded transmission
    HSoundcardWriter<int16_t> soundcard("soundcard output", atoi(argv[2]), H_SAMPLE_RATE_48K, 1, H_SAMPLE_FORMAT_INT_16, BLOCKSIZE, fade.Consumer());
