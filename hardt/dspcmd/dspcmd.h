/**
    ...
    -\subpage bgt = Create biquad filter and run file through the filter
    ...
    \page if name                   Name and path of input file
    \page od device                 Output audio device
    \page of name                   Name and path of output file
    \page force                     Force overwrite of existing file
    \page r rate                    Sample rate (H_SAMPLE_RATE_8K, H_SAMPLE_RATE_11K, H_SAMPLE_RATE_22K, H_SAMPLE_RATE_32K, H_SAMPLE_RATE_44K1, H_SAMPLE_RATE_48K, H_SAMPLE_RATE_96K, H_SAMPLE_RATE_192K) (default 48000)
    \page start [YYYY-MM-DD ]hh:mm  Start time
    \page stop [YYYY-MM-DD ]hh:mm   Stop time

    \page sg freq phase duration    Run as signalgenerator, duration in seconds

    \page nc server port            Run as network client, reading from the network and writing to a local writer
    \page ns port                   Run as network server, reading from a local reader and writing to the network

    \page rf                        Record file
    \page pf                        Play file

    \page fmp size                  Run FFT on a file and plot the magnitude spectrum on screen
    \page fmgp size                 Run FFT on a file and plot the magnitude spectrum on screen using GnuPlot
    \page avg number                Take 'number' FFT's and return the average spectrum
    \page pr xmin xmax              Set minimum and maximum on the x-axis when plotting (only GnuPlot)
    \page pr xmin xmax ymin ymax    Set minimum and maximum on the x-axis when plotting (only GnuPlot)

    \page fc in-format out-format   Convert input file format to output file format (same as in -ff)

    \page mx file1 file2            Mix (linear) file1 and file2
    \page mp frequency              Multiply (mix nonlinear) with localoscilator signal running at frequency

    \page flt name coeffs           Read coefficients from coeffs and run file through filter name
    \page flp name coeffs           Read coefficients from coeffs and plot filter response for filter name
    \page flgp name coeffs          Read coefficients from coeffs and plot filter response for filter name using GnuPlot

    \page bq name Fcutoff Q G       Create biquad filter, dump coefficients and run sweep
    \page bgt name Fcutoff Q G      Create biquad filter and run file through the filter

    \page bqc coeffs                Create biquad filter, dump coefficients and run sweep
    \page bgct coeffs               Create biquad filter and run file through the filter

    \page g gain                    Run file through gain filter

    \page cb fBase alpha            Sweep a comb filter with base frequency fBase and alpha (gain)
    \page cbt fBase alpha           Run a file through a comb filter with base frequency fBase and alpha (gain)

    \page hm fBase fCutoff          Sweep a hum filter with base frequency fBase and cutoff frequency fCutoff
    \page hmt fBase F G Q           Run a file through a hum filter with base frequency fBase and cutoff frequency fCutoff

    \page gz                        Sweep a file by running the individual bins through a Goertzl filter
